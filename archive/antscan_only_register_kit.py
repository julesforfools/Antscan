...
special_ind = [506, 1196, 931, 1336] #indices from specimens with errors
shmecimens = []# for subsetting if time is limited
for i in special_ind:
    shmecimens.append(specimens[i])
del specimens
specimens = shmecimens
...
with open(sys.argv[3]+"specimens_registered_5X_special.txt", 'w') as fp:
    for item in specimens:
        # write each item on a new line
        fp.write("%s\n" % item)
    print('Done')
