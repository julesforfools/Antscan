...
def mask2crop2(image, mask, dil_it=10):
    ...
    if min_z < 0:
        warnings.warn(f"Warning: Buffer exceeds minimum index along z-axis. Setting min_z to 0.")
        min_z = 0
    if min_y < 0:
        warnings.warn(f"Warning: Buffer exceeds minimum index along y-axis. Setting min_y to 0.")
        min_y = 0
    if min_x < 0:
        warnings.warn(f"Warning: Buffer exceeds minimum index along x-axis. Setting min_x to 0.")
        min_x = 0
...
specimens = specimens[:1]# for subsetting if time is limited, first number must be the same as ind
